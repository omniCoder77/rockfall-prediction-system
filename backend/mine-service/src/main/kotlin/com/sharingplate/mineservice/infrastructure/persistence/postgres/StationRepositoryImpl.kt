...
class StationRepositoryImpl(private val r2dbcEntityTemplate: R2dbcEntityTemplate) : StationRepository {
    ...
    override fun findStationById(stationId: UUID): Mono<Station> {
        ...
        return r2dbcEntityTemplate.select(Query.query(Criteria.where("station_id").`is`(stationId)), Station::class.java)
            .singleOrEmpty()
            .doOnSuccess { station ->
    ...
    override fun addStation(station: Station): Mono<Station> {
        ...
        return r2dbcEntityTemplate.insert(station)
            .doOnSuccess { addedStation -> logger.info("Successfully added station with ID: {}", addedStation.stationId) }
    ...
    override fun updateStation(station: Station): Mono<Station> {
        ...
        return r2dbcEntityTemplate.update(station)
            .doOnSuccess { updatedStation -> logger.info("Successfully updated station with ID: {}", updatedStation.stationId) }
    ...
    override fun deleteStation(stationId: UUID): Mono<Void> {
        ...
        return r2dbcEntityTemplate.delete(Query.query(Criteria.where("station_id").`is`(stationId)), Station::class.java)
